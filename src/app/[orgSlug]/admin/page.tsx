...
import { AdminSetupWizardTab } from '@/components/admin/tabs/AdminSetupWizardTab';
import { AdminClassesSubjectsTab } from '@/components/admin/tabs/AdminClassesSubjectsTab';
import { AdminRoomsTab } from '@/components/admin/tabs/AdminRoomsTab';
import { AdminTeachersTab } from '@/components/admin/tabs/AdminTeachersTab';
import { AdminStudentsTab } from '@/components/admin/tabs/AdminStudentsTab';
import { AdminTimetableTab } from '@/components/admin/tabs/AdminTimetableTab';
import { AdminEvaluationsTab } from '@/components/admin/tabs/AdminEvaluationsTab';
import { AdminGradesTab } from '@/components/admin/tabs/AdminGradesTab';
...
function AdminPageContent() {
    ...
    const createTeacher = async (formData?: any): Promise<string | undefined> => {
        ...
        if (!fnTrim || !lnTrim) { toast.error('Nom et prénom obligatoires'); return undefined; }
        ...
        try {
            ...
            const { data, error } = await supabase.from('teacher_profiles').insert({
                ...
                first_name: fnTrim,
                last_name: lnTrim,
                speciality: formData?.speciality || tSpec || null,
                email: formData?.email || tEmail || null,
                phone: formData?.phone || tPhone || null,
                nationality: formData?.nationality || tNat,
                marital_status: formData?.marital_status || tMarital,
                children_count: formData?.children_count ?? (parseInt(tChildren) || 0),
                residence: formData?.residence || tRes || null,
                access_code: code,
                pin_set: false
            }).select().single();
            if (error) throw error;
    ...
    const createStudent = async (formData?: any): Promise<string | undefined> => {
        const fnTrim = (formData?.first_name || sFN).trim();
        const lnTrim = (formData?.last_name || sLN).trim();
        const clsId = formData?.classroom_id || sClsId;
        if (!fnTrim || !lnTrim || !clsId) { toast.error('Nom, prénom et classe obligatoires'); return undefined; }
        ...
        try {
            ...
            const { data, error } = await supabase.from('student_profiles').insert({
                ...
                first_name: fnTrim,
                last_name: lnTrim,
                sex: formData?.sex || sSex,
                birth_date: formData?.birth_date || sBirth || null,
                classroom_id: clsId,
                phone: formData?.phone || sPhone || null,
                guardian_name: formData?.guardian_name || sGuardian || null,
                guardian_phone: formData?.guardian_phone || sGuardianPhone || null,
                nationality: formData?.nationality || sNat,
                residence: formData?.residence || sRes || null,
                matricule: mat,
                access_code: code,
                pin_set: false,
                ...
                <div className="p-3 sm:p-4 md:p-6 max-w-5xl w-full">
                    ...
                    {tab === 'students' && (
                        <AdminStudentsTab
                            ...
                            cls={cls}
                            filieres={filieres}
                            inscRequests={inscRequests}
                            setInscRequests={setInscRequests}
                            saving={saving}
                            ...
                            setSuspendModal={setSuspendModal}
                            setEmailModalOpen={setEmailModalOpen}
                            exportStudentBulletinPdf={exportStudentBulletinPdf}
                            openCertForStudent={openCertForStudent}
                            exportReleveNotesPdf={exportReleveNotesPdf}
                        />
                    )}
                    ...
                    {tab === 'evaluations' && (
                        <AdminEvaluationsTab
                            ...
                            onNavigateToGrades={(ev: any) => { setGrSelEval(ev); setTab('grades'); }}
                        />
                    )}
                    ...
                    {tab === 'grades' && (
                        <AdminGradesTab
                            grEvals={grEvals}
                            grSelEval={grSelEval}
                            setGrSelEval={setGrSelEval}
                            loadGradeEntries={loadGradeEntries}
                            grGrades={grGrades}
                            setGrGrades={setGrGrades}
                            students={students}
                            saving={saving}
                            saveGradeEntries={saveGradeEntries}
                            exportGradesPdf={exportGradesPdf}
                        />
                    )}

                    {tab === 'whatsapp' && (
                        <AdminWhatsAppQueueTab
                            waQueue={waQueue}
                            waLoading={waLoading}
                            loadWhatsAppQueue={loadWhatsAppQueue}
                            waFilter={waFilter}
                            setWaFilter={setWaFilter}
                            waTargetMode={waTargetMode}
                            setWaTargetMode={setWaTargetMode}
                            waTargetStudent={waTargetStudent}
                            setWaTargetStudent={setWaTargetStudent}
                            waTargetClass={waTargetClass}
                            setWaTargetClass={setWaTargetClass}
                            waCustomMessage={waCustomMessage}
                            setWaCustomMessage={setWaCustomMessage}
                            sendCustomWhatsAppBroadcast={sendCustomWhatsAppBroadcast}
                            waSending={waSending}
                            students={students}
                            cls={cls}
                        />
                    ...
                    {tab === 'modeles' && (
                        <AdminPdfTemplatesTab
                            currentTerm={currentTerm}
                            setCurrentTerm={setCurrentTerm}
                            selBulletinTemplate={selBulletinTemplate}
                            setSelBulletinTemplate={setSelBulletinTemplate}
                            selReceiptTemplate={selReceiptTemplate}
                            setSelReceiptTemplate={setSelReceiptTemplate}
                    ...
                    {tab === 'monitoring' && org && (
                        <AdminMonitoringTab
                            ...
                            teachers={teachers}
                            adminSkyPoints={adminSkyPoints}
                            unlockMonitoring={unlockMonitoring}
                        />
                    )}
                    ...
                    {tab === 'certificates' && (
                        <AdminCertificatesTab
                            ...
                            sStampUrl={sStampUrl}
                            onNavigateTab={(t: any) => setTab(t)}
                            requirePinVerification={(docName, cost, onApproved) => {
                                requestPinProtectedAction({ docName, cost, onApproved });
