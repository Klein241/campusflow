...
interface DayData {
    ...
    title: string;
    theme: string;
    courses_reading: { reference: string; passage: string };
    prayer_focus: string[];
    meditation: string;
...
export default function ContentPage() {
    ...
    const [newDay, setNewDay] = useState<Partial<DayData>>({
        title: '',
        theme: '',
        courses_reading: { reference: '', passage: '' },
        prayer_focus: [''],
        meditation: '',
        practical_action: '',
        is_active: true
    ...
    const fetchDays = async () => {
        ...
        try {
            ...
            const mappedDays: DayData[] = (data || []).map((d: any) => ({
                ...
                title: d.title,
                theme: d.theme,
                courses_reading: typeof d.courses_reading === 'string'
                    ? JSON.parse(d.courses_reading)
                    : (d.courses_reading || { reference: '', passage: '' }),
    ...
    const handleSeed = async () => {
        ...
        setIsSeeding(true);
        try {
            const rows = curriculumData.map(d => ({
                day_number: d.day,
                title: d.title,
                theme: d.theme,
                courses_reading: d.coursesReading,
    ...
    const handleAddDay = async () => {
        ...
        try {
            const dayData = {
                ...
                courses_reading: newDay.courses_reading || { reference: '', passage: '' },
        ...
        try {
            const { error } = await supabase
                ...
                .update({
                    ...
                    courses_reading: editingDay.courses_reading,
                                ...
                                <div className="grid gap-4 sm:grid-cols-2">
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            value={newDay.courses_reading?.reference || ''}
                                            onChange={(e) => setNewDay(prev => ({
                                                ...prev,
                                                courses_reading: { ...prev.courses_reading!, reference: e.target.value }
                                            }))}
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            value={newDay.courses_reading?.passage || ''}
                                            onChange={(e) => setNewDay(prev => ({
                                                ...prev,
                                                courses_reading: { ...prev.courses_reading!, passage: e.target.value }
                                            }))}
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4 py-4">
                                    ...
                                    <div className="grid gap-4 sm:grid-cols-2">
                                        <div className="space-y-2">
                                            <Label>Référence biblique</Label>
                                            <Input
                                                value={editingDay.courses_reading?.reference || ''}
                                        ...
                                        <div className="space-y-2">
                                            <Label>Passage</Label>
                                            <Input
                                                value={editingDay.courses_reading?.passage || ''}
                                                onChange={(e) => setEditingDay(prev => prev ? {
                                                    ...prev,
                                                    courses_reading: { ...prev.courses_reading, passage: e.target.value }
                                                } : null)}
