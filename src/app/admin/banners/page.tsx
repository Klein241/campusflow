...
const AD_PLACEMENTS = [
    ...
    {
        value: 'home_feed',
        label: '📚 Accueil ressources',
        description: 'Bannière en haut de la page ressources',
        icon: BookOpenCheck,
...
export default function AdminAdsPage() {
    const [ads, setAds] = useState<ResourceBanner[]>([]);
    const [isLoading, setIsLoading] = useState(false);
    const [isAdDialogOpen, setIsAdDialogOpen] = useState(false);
    const [editingAd, setEditingAd] = useState<ResourceBanner | null>(null);
    ...
    const fetchAds = async () => {
        setIsLoading(true);
        try {
            const { data } = await supabase.from('resource_banners').select('*').order('display_order');
            if (data) setAds(data);
    ...
    const saveAd = async () => {
        ...
        try {
            ...
            if (editingAd) {
                const { error } = await supabase.from('resource_banners').update(payload).eq('id', editingAd.id);
                if (error) throw error;
            } else {
                const { error } = await supabase.from('resource_banners').insert(payload);
                if (error) throw error;
            }
    ...
    // ── Toggle Active ────────────────────────
    const toggleActive = async (ad: ResourceBanner) => {
        try {
            await supabase.from('resource_banners').update({ is_active: !ad.is_active }).eq('id', ad.id);
    ...
    // ── Open Form ────────────────────────
    const openAdForm = (ad?: ResourceBanner) => {
        if (ad) {
            setEditingAd(ad);
            setAdForm({
    ...
    // ── Reset View Count ────────────────────────
    const resetStats = async (ad: ResourceBanner) => {
        if (!confirm('Remettre les statistiques à zéro pour cette publicité ?')) return;
        await supabase.from('resource_banners').update({ click_count: 0, view_count: 0 }).eq('id', ad.id);
