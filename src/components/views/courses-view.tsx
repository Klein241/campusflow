...
/**
 * PAROLE — courses LSG 1910
 * =======================
 * Premium courses experience redesigned with warm parchment theme,
...
import { coursesApi, coursesBook, courses_BOOKS, DEFAULT_TRANSLATION } from '@/lib/unified-courses-api';
import { usecoursesFavorites, usecoursesHighlights, HIGHLIGHT_COLORS, HighlightColor, shareVerse } from '@/lib/courses-features';
import { useReadingPlanStore, READING_PLANS, MemoryCard, SRSRating } from '@/lib/reading-plans';
import '@/styles/courses-parole.css';
...
export function CoursesView() {
    const { user, CoursesViewTarget, setCoursesViewTarget, setcoursesNavigation } = useAppStore();
    ...
    const [books] = useState<coursesBook[]>(courses_BOOKS);
    ...
    // NAVIGATION
    // ═══════════════════════════════════════
    const selectBook = useCallback(async (book: coursesBook, chapter?: number) => {
        setActiveBook(book);
        const chs = coursesApi.getChapters(book.id);
        setChapters(chs);
        const ch = chapter || 1;
        setActiveChapter(ch);
    ...
    const loadChapter = async (bookId: string, chapter: number) => {
        setLoading(true);
        try {
            const data = await coursesApi.getChapterContent(bookId, chapter, DEFAULT_TRANSLATION);
                            ...
                            <nav className="parole-topnav">
                                <div className="parole-topnav-left">
                                    <div className="parole-breadcrumb" style={{ cursor: 'pointer' }} onClick={() => setShowBookSelector(true)}>
                                        <span className="parole-breadcrumb-book">{activeBook?.name || 'courses'}</span>
...
export type { ParoleScreen as coursesTab };
export { coursesSearch } from '@/components/views/courses-search';
