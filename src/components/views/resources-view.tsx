...
function BookCover({ title, category }: { title: string; category: string }) {
    const gradients: Record<string, string> = {
        'courses-study': 'from-blue-600 to-indigo-800',
...
export function ResourcesView() {
    ...
    const setResourceRating = useCallback(async (bookId: string, rating: number) => {
        if (!user?.id) { setShowAuthGate(true); return; }
    ...
    const shareBook = useCallback(async (book: Book) => {
        ...
        const shareText = `📚 ${book.title}\npar ${book.author}\n\nDécouvrez ce livre sur CentreFormation Pro !`;
            ...
            <div className="relative min-h-screen bg-linear-to-b from-[#0B0E14] to-[#0F1219] text-white pb-24 overflow-y-auto">
                <div className="relative z-10 max-w-lg mx-auto w-full px-4 pt-4">
                    <Button size="sm" variant="ghost" className="text-slate-400 hover:text-white mb-4"
                        ...
                        <ArrowLeft className="h-4 w-4 mr-1" /> ressources
                    </Button>
                    ...
                    <div className="flex gap-4 mb-6">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div>
                                ...
                                <StarRating
                                    rating={userRating}
                                    onChange={isLoggedIn ? (r) => setResourceRating(selectedBook.id, r) : undefined}
                                    size="lg"
                                />
                            </div>
                        </div>
                    ...
                    {(() => {
                        ...
                        const items: Array<{ type: 'book'; data: Book } | { type: 'ad'; data: ResourceBanner }> = [];
                                ...
                                <div className="grid grid-cols-3 gap-2.5">
                                    {items.map((item, i) => {
                                        if (item.type === 'book') {
                                            ...
                                            const ad = item.data as ResourceBanner;
                                            return (
                                                <a
                                                    key={`a-${ad.id}`}
                                                    href={ad.link_url || '#'}
                                                    ...
                                                    onClick={() => {
                                                        // Track click
                                                        supabase.from('resource_banners').update({ click_count: (ad as any).click_count + 1 }).eq('id', ad.id).then(() => { });
                                                    }}
                                                >
            ...
            <div className="relative z-10 max-w-lg mx-auto w-full px-4 pt-6">
                ...
                {isLoading ? (
                    <div className="flex flex-col items-center justify-center py-16">
                        ...
                        <p className="text-sm text-slate-400">Chargement de la ressources...</p>
                    </div>
